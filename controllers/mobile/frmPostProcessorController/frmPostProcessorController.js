define({ 
  ...
  */
  getStores : function (eventobject) {
    kony.print("Entering into getStores");
    // Retrieve the user selected newsType and validate it
    var city = "Washington";
    kony.print("User selected city is: "+city);

    // If newsType is valid then proceed with invoking the integration
    // service call
    kony.print("city is valid, proceeding with invoking FoxNews integration service.");
    serviceName = "BestBuyTraining";
    var integrationObj = this.getIntegrationServiceObject(serviceName);
    operationName =  "GetStoreLocationsByCity";
    data = {"city": city};
    headers = {};
